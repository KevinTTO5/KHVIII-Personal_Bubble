...
def run_detective_direct(entities):
    ...
    try:
        ...
        print(f"[detective] GenAI call failed: {e}")
        return []

    text = extract_genai_text(response)
    if not text:
        print("[detective] Empty LLM response")
        return []
...
# ---- helpers: robust extraction + safe parsing ----

def extract_genai_text(resp) -> str:
    """
    Try multiple known shapes from google.genai responses.
    Returns '' if nothing textual is present.
    """
    if resp is None:
        return ""

    # Newer genai clients often expose a convenience field:
    t = getattr(resp, "output_text", None)
    if t:
        return t

    # Some variants expose .text directly
    t = getattr(resp, "text", None)
    if t:
        return t

    # Candidates-style structure
    cand = getattr(resp, "candidates", None)
    if cand:
        # Try to join all text parts we can find
        chunks = []
        for c in cand:
            # c.content.parts[i].text
    ...
    # Some SDKs expose resp.output[0].content parts
...
def parse_jsonl(text: str):
    """
    Accepts JSON Lines or a single JSON array. Null-safe.
    Returns a list[dict].
    """
    if not text:
        return []
    ...
    # If the model returned a fenced block, strip it.
    if text.startswith("```"):
        # take the content between the first and last fences
    ...
    # If it's a single JSON array, parse as one
