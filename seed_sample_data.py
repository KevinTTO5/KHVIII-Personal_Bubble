...
# seed_presence_windows.py
# Seeds Firestore with 10 presence_windows documents and updates entities/ for each tracked id.
# Uses your desired schema for both collections.

# seed_presence_windows.py
# Creates 10 presence_windows intervals for Firebase using only the new data format.
...
def write_presence_window(doc: dict):
    # Convert ISO times to Firestore Timestamps for start_time and end_time
    win = dict(doc)
    win["start_time"] = to_dt(doc["start_time"])
    win["end_time"]   = to_dt(doc["end_time"])
    db.collection("presence_windows").add(win)
...
def seed():
    ...
    for i in range(1, 11):  # interval_id: 1..10
        ...
        present = []
        # id 1: mostly always present
        if random.random() < 0.9:
            present.append(1)
        # id 2: disappears for a couple intervals, then reappears
        if i not in (4, 5):
            if random.random() < 0.75:
                present.append(2)
        if random.random() < 0.5:
        ...
        # people_data uses your exact export format now
        people_data = []
        for pid in sorted(set(present)):
            appear_counts[pid] += 1
            base = 3.5 if prev_present[pid] else 2.0
            dur = max(0.6, min(5.0, random.gauss(base, 0.8)))
            people_data.append({
                "id": pid,
                "reappearance_counter": appear_counts[pid],
                "time": round(dur, 2)
        ...
        write_presence_window(interval_doc)
    ...
    print("\n✅ Seed complete. Check Firestore presence_windows collection for new data.")
